...
function keys(e) {
	let code = e.code;
	if (isFinished) {
	  // Если персонаж уже на финише, игнорируем нажатие кнопок
	  return;
	}
	switch (code) {
	  //arrows
  ...
  bu.addEventListener("click", (e) => {
	if (isFinished) {
	  // Если персонаж уже на финише, игнорируем нажатие кнопки
	  return;
	}
	up();
	firstMove = true;
  ...
  function up() {
	if (isFinished) {
	  // Если персонаж уже на финише, игнорируем вызов функции
	  return;
	}
	animKeys(bu);
	if (checkYboundry("u")) {
...
function updateEmo(lr) {
  ...
  if (lr) {
    if (thingie.offsetLeft > mazeWidth) {
      ...
      thingie.style.position = 'static';
      isFinished = true; // Персонаж достиг финиша
      return; // Прерываем функцию, если персонаж уже на финише
    }
    prevl = thingie.offsetLeft;
  }

  // Проверка на касание и отображение сообщения о победе
  if (thingie.offsetLeft > (mazeWidth - step) && thingie.offsetTop === home.offsetTop) {
    let victoryElement = document.querySelector('.victory');
    if (victoryElement.innerHTML === "") {
      victoryElement.innerHTML = getRandomText(); // Call function to get random text
